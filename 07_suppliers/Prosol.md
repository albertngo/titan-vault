---
type: supplier
# Margin notes are cost data by design — see CONVENTIONS: Visibility.
visibility: admin
status: active
last_activity: 2026-09-18
outlook_message_id: "<c5799c389a46412b11795d856.b708b9e8da.20260728144520.ca6f7979b2.8efa9caa@mail152.atl21.rsgsv.net>"
---

# Prosol

**Rep / contact:** via `info@` email thread
**Terms:**
**Margin notes:** <!-- e.g. BIYORK near break-even, VIDAR volume trap -->

## Products carried
<!-- high level; live data stays in Airtable catalogue -->

## Log
- 2026-08-02 — Created from [[Outlook]] catch-up ingest. **2026-07-28 — price
  increase notice:** several of Prosol's own suppliers have announced increases
  that Prosol is passing through. Affects catalogue pricing and any open quotes
  built on current cost — re-check quotes still outstanding before honouring
  them.
- 2026-08-02 — [[Outlook]]: open online order **#6648646** (ref #340171, placed
  2026-06-29) — Prosol asked for an estimated pick-up date on 07-30 and was
  answered the same day from `info@`. Order still awaiting pickup as of the
  ingest window; note Prosol's Civic Holiday context, Monday 2026-08-03 is a
  closed day across most suppliers.
- 2026-08-15 — [[Outlook]] daily ingest: another price-increase notice from
  Prosol (2026-08-14, `info@`) — some of Prosol's own suppliers announced rough
  percentage increases across most products, being passed through; no
  attachment, no immediate ask. Same pattern as the 2026-07-28 notice above —
  re-check any quotes still outstanding before honouring current-cost pricing.
- 2026-09-05 — [[Outlook]] daily ingest: another "Price Increase Notice" from Prosol (2026-09-04, `info@`) listing pricing changes by manufacturer and product for September 2026 — no attachment noted, feeds the Airtable catalogue workflow. Same pattern as the 2026-07-28/08-14 notices above — re-check any quotes still outstanding before honouring current-cost pricing.
- 2026-09-18 — Outlook daily ingest (email dated 2026-09-15 13:46 to info@): invited to a LATICRETE installation-materials product training day. Informational, optional — no immediate action.
