---
type: client
visibility: staff
status: active
last_activity: 2026-08-02
# Source-system IDs — the join keys. Names are display; IDs are identity.
# Agents match on these BEFORE name, so a rename in GHL never creates a duplicate note.
ghl_contact_id: none — not in today's ingest; Notion payment row only (see Log)
ghl_conversation_ids: []
# Operational, not identity: who owns this record in GHL right now (name + id).
# Used to decide whom the next task gets assigned to. Absent = unassigned in GHL.
ghl_assigned_to:
ghl_assigned_to_id:
---

# Elena Passetchnaia

**Contact:** —
**Address:** —
**Source:** — (customer; surfaced via Notion Master Payments Log)

## Context
<!-- human-owned: who they are, what they want, quirks -->

## Opportunities
None noted yet (project predates vault coverage).

## Log
- 2026-07-28 — created on [[Notion]] daily ingest: $3,000.00 Interac e-Transfer received 2026-07-27, auto-deposited (ref C1AHhVspdE2Y). No project linked on the payment row. Payment: https://app.notion.com/3ab596a4505f817c8213e42eda1873f1. Related: a "Create Gift Box For: [Elena Pass]" tactical task has sat Not started in Notion since 2026-07-07 (20 days, stale) — assumed to be the same person.
- 2026-07-28 — Project-status meeting: herringbone project finished Jul 27, zero deficiencies so far. $700 change-order to be folded into the project total in Notion (duplicate scheduling row to delete); payout starting.
- 2026-07-29 — Notion daily ingest: a **second** $3,000.00 Interac e-Transfer received Jul 28, auto-deposited (ref C1AkkcQwq9YG — distinct from Jul 27's C1AHhVspdE2Y; two real transfers, not a duplicate row). Not yet linked to a project row; the Jul 27 one now is (Elena Pass | Mississauga). Payout phase underway. Payment: https://app.notion.com/3ac596a4505f819ba887d08a69e83e26
- 2026-08-02 — [[Outlook]] catch-up ingest: e-Transfer receipts forwarded to `info@` show **three** transfers from Elena in the 07-26 → 08-02 window — $3,000, $3,000 and **$920.31**. The two $3,000 transfers are the Jul 27 / Jul 28 payments already logged from [[Notion]]; the **$920.31 is new and not yet seen in the Master Payments Log** — a third, smaller transfer, unexplained here (the $700 herringbone change-order is the obvious candidate but the numbers do not tie). Confirm it is recorded and linked to the project row. Joined by name from email only, not by a payment-row ID. #admin
